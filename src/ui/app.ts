...
export async function runChatApp(options: ChatAppOptions): Promise<void> {
  ...
  const checkGithubUpdate = (): void => {
    try {
      const localPath = existsSync("/home/mxgamecoder/baton/.git") ? "/home/mxgamecoder/baton" : "";
      if (localPath) {
        const fetchRes = spawnSync("git", ["-C", localPath, "fetch", "origin", "master"], { encoding: "utf8", timeout: 8000 });
        if (fetchRes.status === 0) {
          const revDiff = spawnSync("git", ["-C", localPath, "rev-list", "HEAD..origin/master", "--count"], { encoding: "utf8" });
          const count = parseInt(revDiff.stdout?.trim() || "0", 10);
          if (count > 0) {
            ui.line(`⚡ ${count} new update(s) detected on GitHub — updating baton…`, theme.accent);
            const res = runUpdate();
            if (res.ok) {
              ui.line("✓ baton updated to latest commit! (restart to take full effect)", "#7fd1b9");
            }
          }
        }
      }
  ...
  setInterval(checkGithubUpdate, 180000).unref?.();
